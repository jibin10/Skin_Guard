...
export class ConsultationListComponent implements OnInit, OnDestroy {
  ...
  constructor(public consultationService: ConsultationService, private authService: AuthService, public dialog: MatDialog) {}

  ngOnInit() {
    this.isLoading = true;
    this.consultationService.getAppointments(this.appointmentsPerPage, this.currentPage);
    this.userId = this.authService.getUserId();
    this.role = this.authService.getRole();
    this.appointmentsSub = this.consultationService.getAppointmentUpdateListener()
  ...
  filterConsultation(appData:AppointmentData[]) {
    ...
    for(let index in appData) {
      if((appData[index].doctor == this.userId && appData[index].status =='open') || (this.role == 'Admin' && appData[index].status =='open')) {
        openData.push(appData[index]);
      }
    }
    return openData;
  }
